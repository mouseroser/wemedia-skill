...
- **打磨层（Step 1.5）v2.5**：Spec-Kit Gate → NotebookLM 上下文 → gemini 扫描 → openai 宪法 → claude 实施计划 → review/gemini 复核 → review/openai|claude 仲裁
- **Spec-Kit / NotebookLM 口径**：`Spec-Kit` 是 L2/L3 默认主链 gate，不是可选 companion；`notebooklm` 保留 Step 1.5 知识支撑与 Step 6 模板支撑，失败只允许降级，不允许据此移除架构
- **Step 3**：`review` 编排 `claude + gemini` 双审，分歧时 `openai` 仲裁
- **Step 4**：`brainstorming` 出修复方案 → `coding` 修复 → `review/gemini` 预审 → `review` 正式复审
- **Step 5.5 增强诊断与仲裁（v2.5 延续）**：
  - 1️⃣ gemini/high → 诊断 memo
  - 2️⃣ review/gemini/medium → 复核诊断
  - 3️⃣ brainstorming/opus/high → 根因分析 + 回滚决策
  - 4️⃣ review/openai/high → 仲裁回滚决策（按需：Epoch>=2 / 回滚到S1 / 信心度<0.7）
...
### 星链流水线 v2.5（2026-03-08 恢复）
- 角色分工：Gemini 扫描 + 反方 review；OpenAI 宪法 + 仲裁；Claude 主方案；Review 交叉评审中枢
- Step 1.5D：review spawn gemini 做一致性复核
- Step 1.5E：review spawn openai 做仲裁（按独立性规则可切换 claude）
- Step 3：review spawn claude + gemini 双审，分歧时 spawn openai 仲裁
- Step 4：review spawn gemini 预审
- 模板恢复：Step 1.5A/B 标准模板 + Step 1.5C/D/E 简化模板已恢复到 `intel/starchain-templates/`
