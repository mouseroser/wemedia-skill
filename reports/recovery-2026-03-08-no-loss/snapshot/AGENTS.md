...
- **Step 1.5**: Spec-Kit Gate + Constitution-First（Spec-Kit → NotebookLM 上下文 → gemini 扫描 → openai 宪法 → claude 计划 → review/gemini 复核 → review/openai|claude 仲裁）
- **Step 2-7**: 串联所有步骤，直接 spawn 各 agent
- **Step 3**: review 编排 `claude + gemini` 双审，分歧时 `openai` 仲裁
- **Step 4**: `brainstorming` 出修复方案 → `coding` 执行修复 → `review/gemini` 预审 → `review` 正式复审
- **Step 5.5**: 增强 Epoch 诊断与仲裁（`gemini` 诊断 → `review/gemini` 复核 → `brainstorming` 根因分析 → `review/openai` 仲裁决策）
- **Step 7**: 汇总交付 + 通知晨星
...
- **Step 5**: review/openai 按需仲裁（高风险场景）
...
### Type A/B 动态模型切换
- **coding / test** 仍按任务类型动态覆盖模型
- **review** 在 v2.5 中固定为编排壳层：`claude + gemini` 双审，分歧时 `openai` 仲裁
- **仲裁独立性**：谁主写，谁尽量不终审；必要时在 `openai / claude` 间切换仲裁者
...
#### 标准前置链（按复杂度启用）
- **L1**：Fast Lane，默认跳过 Step 1.5 与 Step 6
- **L2/L3**：先过 `Spec-Kit Gate`，再进入 Constitution-First 前置链
- **Constitution-First**：`gemini` 扫描 → `openai` 宪法 → `claude` 计划 → `review/gemini` 复核 → 按需 `review/openai|claude` 仲裁

#### 目标
- 先把晨星与系统对问题的理解对齐到同一颗粒度
- 再由 Claude Code 在“最终宪法”上出实施计划
- 先由 `brainstorming` 产出默认 `Spec-Kit` 四件套（`spec.md` / `plan.md` / `tasks.md` / `research.md`）
- 再由 `notebooklm` 提供 Step 1.5 上下文知识补料（失败可降级，不得据此移除架构）
- 再由 Gemini 做一致性复核
- 仅在高风险、明显分歧或 L3 场景下启用 `review/openai|claude` 作为仲裁位

#### 执行规则
- **不中断自动化**：不新增晨星中途确认节点，Step 1.5 内部自动推进
- **不替换主干**：此链条是 Step 1.5 的升级版前置链，不替换后续开发 / 审查 / 测试 / 文档 / 交付主干
- **Spec-Kit 默认保留**：进入正式开发前，`brainstorming` 默认产出 `Spec-Kit`；`coding` 默认以 `tasks.md` 作为执行入口
- **NotebookLM 默认保留**：`notebooklm` 保留 Step 1.5 知识支撑（`reports/step-1.5-knowledge.md`）与 Step 6 模板支撑（`reports/step-6-template.md`）
- **OpenAI 职责**：最终宪法 + 默认仲裁
- **Claude 职责**：主计划器 / 主执行规划者
- **Gemini 职责**：前置扫描 + 后置 adversarial review
- **Review 职责**：交叉评审中枢，编排 `gemini / openai / claude`
